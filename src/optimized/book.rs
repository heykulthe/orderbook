...
#[allow(dead_code)]
const fn bool_to_usize(b: bool) -> usize {
    b as usize
}
...
impl L2Book {
    ...
    fn set_bid_level(&mut self, price: Price, qty: f32) {
        ...
        let sanitized_qty = if qty.is_finite() && qty > EPS {
            qty
        } else {
            0.0
        };
    ...
    #[cfg_attr(feature = "no_checksum", allow(unused_variables))]
    pub fn update(&mut self, msg: &L2UpdateMsg, symbol: &str) -> bool {
        ...
        //
        // Validation is placed *after* applying the diffs. This is intentional:
        //
        //   • In "A" (Benchmark Mode), the checksum is a full BBO-based Adler32
        //     hash computed against the already-updated book state. Placing it
        //     post-mutation means the hash always reflects the current book,
        //     giving a deterministic and reproducible workload for benchmarking.
        //
        //   • In "B" (Production Mode), the O(1) sequence continuity check is
        //     side-effect free, so mutation ordering has no impact on correctness.
        //     Keeping the control-flow structure unified avoids introducing
        //     branch divergence in the hot path.
        //
        // Summary:
        //   - Benchmark Mode  => full BBO hash for reproducible perf measurements
        //   - Production Mode => O(1) integer continuity check, zero allocation
        //
        // The two paths are isolated behind `cfg` flags so neither bleeds into
        // the other at compile time.
        //

        #[cfg(not(feature = "no_checksum"))]
        {
            // ----- Mode A: Benchmark Hashing ---------------------------------------
            //
            // Computes the "symbol|seq|best_bid|best_ask" Adler32 hash against
            // the post-update book state. Both BBO reads are O(1) via `best_rel`,
            // and integer formatting uses `itoa` stack buffers — no heap allocation.
            //
    ...
    /// Computes and verifies the BBO-based Adler32 state checksum.
    ///
    /// Compiled only in benchmark mode (default build, without `no_checksum`).
    /// This is the "A" path of the dual-mode validation switch in `update()`.
    ///
    /// The hash payload is `"symbol|seq|best_bid_tick|best_ask_tick"` encoded
    /// as ASCII bytes and fed into an Adler32 accumulator.
    ///
    /// # Performance Properties
    ///
    /// 1.  **O(1) BBO Reads:**
    ///     Both `best_bid()` and `best_ask()` read the cached `best_rel` field —
    ///     a single L1d load with no scan or allocation.
    ///
    /// 2.  **`#[cold]` / `#[inline(never)]`:**
    ///     Marked cold to prevent this auxiliary path from polluting the L1
    ///     instruction cache used by the `update()` hot path.
    ///
    /// 3.  **Zero Heap Allocation:**
    ///     Integer formatting uses `itoa::Buffer` (stack-allocated) rather than
    ///     `format!` (heap), keeping allocator jitter out of the measured path.
    ///
    #[cfg(not(feature = "no_checksum"))]
    #[cold]
    #[inline(never)]
...
mod tests {
    ...
    fn test_recenter_threshold() {
        ...
        #[cfg(debug_assertions)]
        {
            ...
            assert_eq!(
                recenters_after, initial_recenters,
                "Should not recenter within margin"
            );
        }
    }
    ...
    fn test_massive_wraparound() {
        ...
        assert!(
            best.0 >= 49000 && best.0 <= 51000,
            "Best price should be in reasonable range"
        );
        // With CAP=4096 and recentering thresholds, expect at least 200 levels maintained
        assert!(
            book.bid_depth() >= 200,
            "Should have many levels after wraparound, got {}",
            book.bid_depth()
        );
    }
    ...
    fn test_large_price_jump_reseed() {
        ...
        assert!(
            book.bid_depth() <= 10,
            "Old levels should be cleared after reseed"
        );
    }
    ...
    fn test_band_clearing_after_recenter() {
        ...
        let jump = L2UpdateMsg {
            ...
            diffs: vec![L2Diff {
                side: Side::Bid,
                price_tick: 50000 + RECENTER_HIGH_MARGIN as i64,
                size: 1.0,
            }],
            checksum: 0,
        };
        ...
        assert!(
            depth <= 10,
            "Old levels should be cleared after recenter, got depth={}",
            depth
        );
    }
    ...
    fn test_no_infinite_recursion() {
        ...
        let recenter_and_update = L2UpdateMsg {
            ...
            diffs: vec![
                L2Diff {
                    side: Side::Bid,
                    price_tick: 50000 + RECENTER_HIGH_MARGIN as i64 + 100,
                    size: 5.0,
                },
                L2Diff {
                    side: Side::Bid,
    ...
    fn test_negative_shift_recenter() {
        ...
        assert!(
            best.0 >= 50100,
            "Best bid should be high price, got {}",
            best.0
        );
        ...
        assert!(
            book.bid_depth() >= 50,
            "Should have many levels after negative shift, got {}",
            book.bid_depth()
        );
    }
    ...
    fn test_small_shift_recenter_no_ghost_liquidity() {
        ...
        assert!(
            depth_initial >= 10,
            "Should have at least 10 bid levels, got {}",
            depth_initial
        );
        ...
        if let Some((price, qty)) = new_best {
            ...
            assert!(
                price < shift_price,
                "New best should be below removed price, got {} vs {}",
                price,
                shift_price
            );
            ...
            assert!(
                book.bid_depth() >= 1,
                "If there's a best bid, depth should be >= 1"
            );
            ...
            let is_valid = (price >= 49995 && price <= 50005)
                || (price > shift_price - 100 && price < shift_price);
            assert!(
                is_valid,
                "Price {} appears to be a ghost (outside expected ranges)",
                price
            );
        } else {
            ...
            assert_eq!(
                book.bid_depth(),
                0,
                "If best_bid() is None, bid_depth() should be 0, got {}",
                book.bid_depth()
            );
        }
    }
    ...
    fn test_negative_shift_recenter_no_ghost_liquidity() {
        ...
        assert!(
            depth_after_shift >= 1,
            "Should have at least one level after shift"
        );
        ...
        for (price, qty) in &top_bids {
            assert!(
                *qty > 0.0,
                "All bid quantities should be > 0, found ghost at price {} with qty {}",
                price,
                qty
            );
        }
        ...
        assert_eq!(
            top_bids.len(),
            depth_after_shift,
            "top_bids length should match bid_depth, got {} vs {}",
            top_bids.len(),
            depth_after_shift
        );
    }
    ...
    fn test_wraparound_shift_no_corruption() {
        ...
        assert_eq!(
            book.best_bid(),
            None,
            "Book should be empty after removing all levels"
        );
        assert_eq!(
            book.bid_depth(),
            0,
            "Depth should be 0 after removing all levels"
        );
    }
    ...
    fn test_no_ghost_liquidity_after_soft_recenter() {
        ...
        assert_eq!(
            book.best_bid(),
            Some((50000, 10.0)),
            "Initial bid should be at 50000"
        );
        ...
        assert_eq!(
            bids_before.len(),
            1,
            "Should have exactly 1 bid before recenter"
        );
        ...
        let has_trigger = bids_after
            .iter()
            .any(|(p, q)| *p == trigger_price && (*q - 5.0).abs() < 0.001);
        assert!(
            has_trigger,
            "The trigger price {}@5.0 should be in the book",
            trigger_price
        );
        ...
        println!(
            "Test passed: found_original={}, bids_after={:?}",
